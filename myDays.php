<?php
    ...
    $sql = "SELECT * FROM mydays ORDER BY id DESC";
    $notes = query($sql);

    setlocale(LC_TIME, 'id_ID.utf8');

    if(isset($_POST['submit'])){
        if(insertNote($_POST) > 0){
            echo "<script> alert('Data Berhasil Ditambahkan !'); document.location.href = 'myDays.php'; </script>";
        }else{
            echo "<script> alert('Data Gagal Ditambahkan !'); document.location.href = 'myDays.php'; </script>";
        }
    }

    if(isset($_POST['save'])){
        if(editNote($_POST) > 0){
            echo "<script> alert('Data Berhasil Diedit !'); document.location.href = 'myDays.php'; </script>";
        }else{
            echo "<script> alert('Data Gagal Diedit !'); document.location.href = 'myDays.php'; </script>";
        }
    }
    if(isset($_POST['delete'])){
        $id = $_POST['id'];
        if(deleteNote($id)){
            echo "<script> alert('Data Berhasil Dihapus !'); document.location.href = 'myDays.php'; </script>";
        }else{
            echo "<script> alert('Data Gagal Dihapus !'); document.location.href = 'myDays.php'; </script>";
        }
    }
?>
<!DOCTYPE html>
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            <form action="myNote.php" method="POST">
                                ...
                                <input type="hidden" name="waktu" id="waktu" value="<?= $row['waktu']; ?>">
                                <input type="hidden" name="warna" id="warna" value="<?= $row['warna']; ?>">
                                <input type="hidden" name="judul" id="judul" value="<?= $row['judul']; ?>">
                                <input type="hidden" name="catatan" id="catatan" value="<?= $row['catatan']; ?>">
                                <input type="hidden" name="foto" id="foto" value="<?= $row['foto']; ?>">
                                <div class="card-header" style="background-color: <?= $row['warna']; ?>;">
                                    <h4 class="mb-0 text-white"><?= $row['judul']; ?></h4>
                                </div>
                                <div class="card-body">
                                ...
                                ">
                                        <?= $row['catatan']; ?></p>
                                    <button name="more" class="btn text-white"
                                        style="background-color: <?= $row['warna']; ?>;">More . . .</button>
                                    <div class="mt-4">
                                        <p class="card-text"><code
                                                class="text-muted"><?php
                                            $date = date_create($row['tanggal']);
                                            $time = date_create($row['waktu']);
                                            echo date_format($date,"d F Y") . " at " . date_format($time,"h:i A");  ?></code>
                                        </p>
                                    </div>
                                </div>
                            </form>
